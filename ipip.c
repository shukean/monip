...
static void ipip_split(zval *return_value, const char *str, uint str_len) {
    char *p1, *p2, *endp;
...
static bool ip_find(const char *ip_str, zval *rsp) {
    if (datx_offset < 4) {
    ...
    ipip_split(rsp, datx_buf + datx_offset + index_offset - 262144, index_len);
    return true;
}

/* Every user-visible function in PHP should document itself in the source */
/* {{{ proto string confirm_ipip_compiled(string arg)
...
PHP_FUNCTION(ipip_find)
{
    ...
    bool ret = ip_find(ipstr, return_value);
    if (!ret) {
        RETURN_FALSE;
    }
    return;
...
{
#if defined(COMPILE_DL_IPIP) && defined(ZTS)
